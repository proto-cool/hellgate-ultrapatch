...
```sh
python3 tools/hgdat.py extract /tmp/hg ".fxo"                      # all 225 effects out of the paks
python3 tools/hgfx.py dump /tmp/hg/data_common/effects/dx9/particle.fxo --blobs /tmp/blobs
python3 tools/hgfx.py table /tmp/hg > src/fxtable.h                 # (size, fnv1a) -> pak path, for the DLL
...
## Building replacement effects (learned the hard way, 2026-09-21)

`hgfx.py` can now write fx_2_0 as well as read it (`roundtrip` is byte
identical on all 120 effects except the compiler's uninitialised padding),
and `tools/mkfx.py` clones techniques with an extra pass. What the game's
D3DX (native d3dx9_34) demands, each found by bisecting with `fxload.exe`:

1. **Layout.** The blob is read sequentially: per parameter typedef,
   (sampler states as value, typedef), value, annotations as value /
   typedef / name, then name and semantic; per technique annotations, then
   per pass states as value / typedef and the pass name, then the technique
   name; string and resource sections in descending object id. Same
   offsets in a different order → E_FAIL.
2. **Third header count** = shader states with an object + passes +
   sampler parameters. Undercount it and the large effects fail.
3. **Shader blobs must come from the effect compiler.** Both vkd3d and
   `D3DXCompileShader` produce valid shaders (they create fine as raw
   shaders and their CTABs parse) but no constant default blocks; inside an
   effect, a constant spanning many registers (`Bones[180]`) then fails the
   load. `tools/fxcomp.c` compiles a wrapper `.fx` with
   `D3DXCreateEffectCompiler` from the prefix's d3dx9_34 and the blobs are
   lifted from the result. Patching a CTAB by hand (`hgfx.fix_ctab`) makes
   things worse; do not use it.
4. Non-normal-mapped materials declare no TANGENT/BINORMAL; a pass that
   reads them would draw garbage on those meshes.

`tools/build_shaders.sh` (Makefile `shaders`) does the whole chain and
validates before installing.

## Owning the source (plan step 7, 2026-09-22)

The material effects are now rebuilt from `tools/shaders/actor.hlsl` and
`tools/shaders/background.hlsl`, at pixel parity with stock (`make
matcheck`). The comments at the top of each file say what the stock shaders
do. LOG.md (2026-09-22, "material shader rewrite") lists every quirk that was
reproduced. Workflow: `tools/matcheck.sh <effect> <family>`, which writes
failing image pairs to `build/mat/<effect>/dump/`. Then
`tools/fxcmp.sh <effect> <technique> vs|ps` shows the stock and rebuilt
shaders of that technique side by side (constant table, preshader,
assembly). `hgfx.py pres <blob>` shows a single shader's preshader.
