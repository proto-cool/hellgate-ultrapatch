...
# materials). Run inside the dev toolbox:  toolbox run -c dev tools/build_shaders.sh
#
# Steps: extract the stock effects -> rebuild the actor effects from our own
# source (tools/shaders/actor.hlsl via mkmat.py; parity with stock is checked
# by tools/matcheck.sh, not here; MAT=0 skips this and starts from stock) ->
...
[ -f "$W/hg/data/effects/dx9/actoroutdoor30.fxo" ] || python3 tools/hgdat.py extract "$W/hg" 'effects\dx9\actor'
[ -f "$W/hg/data/effects/dx9/backgroundoutdoor30.fxo" ] || python3 tools/hgdat.py extract "$W/hg" 'effects\dx9\background'
...
if [ "${MAT:-1}" = 1 ]; then
    ...
    tools/matcompile.sh $PAIRS
    for pair in $PAIRS; do
        eff=${pair%%:*}
        python3 tools/mkmat.py build "$W/hg/data/effects/dx9/$eff.fxo" ${pair##*:} build/mat/$eff "$ROOT/data/effects/dx9/$eff.fxo"
    done
fi
( cd tools/shaders
...
python3 - "$W" <<'PY'
import sys, os; sys.path.insert(0, 'tools'); import hgfx
w = sys.argv[1]
...
PY
python3 tools/mkfx.py "$ROOT" "$W/bin" "$W/out"
