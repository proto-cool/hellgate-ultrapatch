...
// the stock bytecode so we own the source (notes/graphics-plan.md step 7).
//
// Stage 1 of the rewrite is PARITY: every technique compiled from this file
// must draw the same pixels as the stock one (tools/fxdiff.c checks all of
// them). Only once that holds do lighting changes go in, behind #if so a
// parity build stays one define away.
//
// One compile per technique feature set; tools/mkmat.py derives the defines
