...
#   tools/matcompile.sh <effect>:<family> ...     e.g. actoroutdoor30:actor
# For each: tools/mkmat.py plan lists the variants whose sources changed
...
for pair in "$@"; do
    ...
    python3 tools/mkmat.py plan "build/shaders/hg/data/effects/dx9/$eff.fxo" "$fam" "$M"
...
ls "$Q"/*.chunk.* | xargs -P "$JOBS" -I{} sh -c '
    ...
    cd tools/shaders && wine ../../build/fxcomp.exe ../../'"$DX"' $fam.fx -batch ../../{} > ../../{}.log 2>&1 || true'
