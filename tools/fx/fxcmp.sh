...
#   tools/fx/fxcmp.sh <effect> <technique> <vs|ps>
# Reads build/shaders/hg/... (stock) and build/mat/<effect>/<effect>.fxo
# (tools/fx/matcheck.sh output); prints each shader's constant table, preshader
# and assembly. Blobs and .asm land in build/mat/{stock,ours}.<vs|ps>.*
set -e
cd "$(dirname "$0")/../.."
EFF=$1; TECH=$2; K=$3
python3 - "$EFF" "$TECH" "$K" <<'PY'
import sys; sys.path.insert(0, 'tools/fx'); import hgfx
...
for t in stock ours; do
    echo "=== $t"
    python3 tools/fx/hgfx.py ctab build/mat/$t.$K.bin | tail -n +2
    python3 tools/fx/hgfx.py pres build/mat/$t.$K.bin | sed 's/^/pres: /'
