...
at a time in shaders/<family>.hlsl, run tools/fx/matcheck.sh, and report
...
exercises that path, so a real regression there would pass unnoticed.
    ...
    toolbox run -c dev python3 tools/fx/matmutate.py [actor|background]
...
def main():
    ...
    uncaught = 0
    for fam in fams:
        src = os.path.join(root, "shaders", fam + ".hlsl")
        good = open(src).read()
        try:
            for name, a, b, eff in MUTATIONS[fam]:
                if good.count(a) != 1:
                    print("%-10s %-24s pattern found %d times; update tools/fx/matmutate.py" % (name, eff, good.count(a)))
                    uncaught += 1
                    continue
                open(src, "w").write(good.replace(a, b))
                out = subprocess.run([os.path.join(root, "tools", "fx", "matcheck.sh"), eff, fam],
