...
`tools/data/hguncook.py`, a stdlib-only Python port of Reanimator-steam's cooked
reader (`ref/Reanimator-steam/hellgate/XmlCookedFile*.cs`). No Mono needed.

```sh
python3 tools/data/hgdat.py extract /tmp/hg "data\lights\"          # get files out
python3 tools/data/hguncook.py "/tmp/hg/data/lights/muzzle flash light.xml.cooked"
python3 tools/data/hguncook.py --defaults <file>   # also list absent fields + defaults
python3 tools/data/hguncook.py --xml <file>        # Reanimator-style XML
python3 tools/data/hguncook.py --check /tmp/hg     # parse everything, report failures
```
