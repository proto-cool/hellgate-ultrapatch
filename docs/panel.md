...
| Post | **HDR scene, tone map, exposure, shoulder, bloom from, scan** | the 3D scene in a float target, tone-mapped before the UI (from the next start); *tone map* off is the stock clamp (A/B); *scan* logs out-of-range pixels |
