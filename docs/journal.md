...
`docs/reference/exe-facts.md`). `dbghelp.dll` is present but that is a crash-dump
...
3. **Havok's own `HK_TIMER` name literals** — `TtRayCstCached`, `TtrcMopp`,
   ...
   entry was found without guessing. (The raw triage output is kept locally, not in the repository.)
...
**Run.** `tools/re/findpat.py` — the pattern from
...
**Run.** `tools/re/callgraph.py` + `tools/up.py` (direct `E8 rel32` xrefs and
absolute pointer scan over the whole image), then Ghidra 12.1.3 headless
(`tools/re/ghidra/*.java`) for function boundaries and decompilation.
...
`docs/logs/baseline-clean-session.log.gz`.
...
19,370 active windows. Archived at `docs/logs/wined3d-session-noQ.log.gz`.
...
Full teardown: **`docs/reference/russian-patch-analysis.md`**. It is a byte patch of
...
`docs/logs/wined3d-session-Q.log.gz`.
...
Archived as `docs/logs/run_{corr,a_continuous,b_discrete}.log.gz`.
...
The brief is `docs/graphics-plan.md`; this is what its step 0 turned up
before the first instrumented run.

**Data.** Reanimator-steam has no Mono here, so its cooked-XML reader was
ported: `tools/data/hguncook.py`, stdlib only. The format turned out to be
...
files parse to EOF. `docs/reference/data-tunables.md` records the fields.

**Shaders.** `tools/fx/hgfx.py` parses the fx_2_0 binaries (layout from Wine's
...
alpha-modulated per-object constant, not depth. `docs/reference/stock-shaders.md`.
...
Graphics answers are in `docs/graphics-plan.md` ("First instrumented run"):
...
`tools/re/ghidra/AssertSurvey.java`): 25,395 functions; 1,978 assert-expression
...
`tools/re/ghidra/NameFromAsserts.java` on `~/ghidra_proj/HG.gpr`: for each of
...
`docs/codemap/` (README + a page per file; `tools/re/codemap.py`), names live
in the Ghidra project, `make codemap` regenerates, `make decomp F="..."`
writes `decomp/<name>.c`.
...
Written into `docs/graphics-plan.md` step 1.
...
lights (`shaders/actor_lights.hlsl`, 16 variants over Skinned /
...
validates every technique), `hgfx.py roundtrip`. All in `docs/reference/stock-shaders.md`.
...
**Run.** New tooling:
- `tools/fx/fxdiff.c` draws every technique of the stock and the rebuilt effect
...
- `tools/fx/mkmat.py` turns each technique's annotations into defines, and
  ...
  byte-for-byte stock.
- `tools/fx/matcheck.sh <effect> <family>` does the whole loop.
  `tools/fx/matmutate.py` breaks one term at a time and confirms the harness
  catches it.
- `tools/fx/hgfx.py pres` decodes preshaders, the CPU-side expressions the
  effect compiler hoists out of shaders (fog scale, specular power range,
  camera-light enable, glow alpha mix).

**Result.** `shaders/actor.hlsl` and `background.hlsl` reproduce all
...
(`shaders/ultra.hlsl`), are added by mkmat with an all-zero default,
...
- the PCSS loops are real loops (a computed Vogel disk instead of a table
  forcing unroll);
- `tools/fx/matcompile.sh` compiles in parallel across all cores;
...
## 2026-09-22 — animation fixes cut

**Decision.** The animation fixes never worked in game and are removed:
`src/animfix.c` (stance ease, ease-out floor, run-cycle seam blend, phase
match, bone spike log), `src/animwatch.c` (the snap detector built for them),
their panel group and tests, and the `hellgate_animtrace.on` flag. The last
in-game report (2026-09-22) was that they still did nothing useful. Earlier
entries: "Animation pass — first in-game report", "Shield: it is the run
cycle's loop seam", "Build 5 in game: ... the seam smoother catching
restarts".
