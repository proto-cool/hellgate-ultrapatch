...
Leads that are not being worked, apart from the graphics roadmap
([graphics-plan.md](graphics-plan.md)). Each is a *lead*, not a finding:
it goes through hypothesis → the log line that would confirm or refute it →
run → conclusion ([journal.md](journal.md)) before anything ships. A patch
project attracts plausible tweaks that were never tested.
...
game's own enum. Walk it with `tools/data/keybinds.py`.
...
dump with handler addresses in `docs/reference/script-actions.md`. Directly useful:
...
demand by replaying a recorded one (see the tooling entry in `docs/journal.md`). The
...
## H. Animation snaps — attempted, cut

**What.** Third-person pops: the Guardian's shield snapping once per run
cycle, and stance transitions that complete in one frame (a zero
`fEaseOut` passed straight to Havok's `easeOut`).

**Evidence.** Confirmed as symptoms; the shield snap is the run cycle's
loop seam (journal, "Shield: it is the run cycle's loop seam"). Animation
is Havok Animation 4.0; Granny 2 is only the model format.

**Status.** Four fixes (stance ease, ease-out floor, seam blend, phase
match) were built and did not work in game; removed 2026-09-22. Code in
commit `e359afe` (`src/animfix.c`, `src/animwatch.c`). A retry should start
by measuring the final pose, not the animation controls.
