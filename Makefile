...
SRC     := src/dllmain.c src/proxy.c src/hook.c src/panel.c src/overlay.c src/device.c src/hdr.c src/postfx.c src/compare.c src/brand.c src/plshadow.c src/volfog.c src/crashlog.c src/cull.c src/diblock.c src/skipprobe.c src/inputfilter.c src/invprobe.c src/uiext.c src/invsort.c src/invplan.c src/settings.c src/optpage.c \
           src/ui.c src/panel_ui.c src/fart.c src/shoulder.c src/fpview.c src/footik.c src/altlatch.c src/gfxprobe.c src/sha256.c $(MH_SRC)
...
build/uitest: test/ui.c src/ui.c src/ui.h src/panel_ui.c src/panel_ui.h src/panel.h src/fart.c src/shoulder.c src/altlatch.c src/inputfilter.c src/invplan.c src/invplan.h | build
...
# The DLL goes in by copy-then-rename: a plain cp rewrote the file a running
# game had mapped, and pages it had not touched yet came from the new build:
# crashes in odd places (2026-09-24). A rename leaves the running game its
# old file; the next launch loads the new one.
install: build/version.dll
	@if [ -d "$(GAME)/bin" ]; then \
	    cp build/version.dll "$(GAME)/bin/version.dll.new" && mv -f "$(GAME)/bin/version.dll.new" "$(GAME)/bin/version.dll" && \
